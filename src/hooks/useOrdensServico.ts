...
export function useOrdensServico() {
  ...
  return useQuery({
    queryKey: ["ordens_servico", companyId],
    enabled: !!companyId,
    staleTime: 30000,
    queryFn: async () => {
  ...
  return useMutation({
    mutationFn: async (input: {
      ...
      if (servicos && servicos.length > 0) {
        const { error: servicosErr } = await supabase.from("os_servicos").insert(servicos.map(s => ({ ...s, ordem_servico_id: data.id, company_id: companyId })));
  ...
  return useMutation({
    mutationFn: async ({ id, pecas, servicos, ...input }: {
      ...
      if (pecas) {
        ...
        if (pecas.length > 0) {
          const { error: pecasErr } = await supabase.from("os_pecas").insert(pecas.map(p => ({ ...p, ordem_servico_id: id, company_id: data.company_id })));
          if (pecasErr) throw pecasErr;
        }
      }
      ...
      if (servicos) {
        ...
        if (servicos.length > 0) {
          const { error: servicosErr } = await supabase.from("os_servicos").insert(servicos.map(s => ({ ...s, ordem_servico_id: id, company_id: data.company_id })));
          if (servicosErr) throw servicosErr;
        }
      }
